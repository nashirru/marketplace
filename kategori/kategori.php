...
if ($category_id) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // Jika kategori tidak ditemukan, redirect ke halaman semua kategori
    ...
    $stmt_prod = $conn->prepare("
        ...
        WHERE p.category_id = ? AND p.is_active = 1 -- Hanya tampilkan produk aktif
        GROUP BY p.id
        -- ✅ LOGIKA PRIORITAS STOK: Stok > 0 (FALSE/0) di atas Stok 0 (TRUE/1)
    ...
    <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
        ...
        <?php if ($category_id): ?>
            ...
            <div class="flex items-center mb-8">
                 <!-- ✅ PERUBAHAN: Tombol Kembali ke Beranda dengan Ikon Saja -->
                 <a href="<?= BASE_URL ?>/" 
                    class="p-2 mr-4 text-indigo-600 bg-indigo-100 rounded-full hover:bg-indigo-200 hover:text-indigo-800 transition shadow-md"
                    title="Kembali ke Beranda">
                     <i class="fas fa-home"></i> 
                 </a>
                 <span class="text-gray-400 text-3xl font-bold">/</span>
                 <h1 class="text-3xl font-bold text-gray-800 ml-4"><?= htmlspecialchars($current_category_name) ?></h1>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
                ...
                <?php else: ?>
                    <div class="col-span-full bg-white text-center p-12 rounded-lg shadow">
                        <p class="font-semibold text-gray-700">Belum ada produk aktif di kategori ini.</p>
                    </div>
                <?php endif; ?>
            </div>

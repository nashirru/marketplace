<?php
// File: sistem/cron_cancel_orders.php
// Skrip ini HANYA untuk dipanggil oleh Cron Job di server Anda.
// Ia akan membatalkan pesanan 'waiting_payment' yang lebih tua dari 24 jam.
...
$CRON_SECRET_KEY = "WarokKitePalingMantap7788";
...
echo "===== Mulai Cron Job Pembatalan Pesanan =====\n";
echo "Waktu Eksekusi: " . date('Y-m-d H:i:s') . "\n\n";

// 1. Cari semua order yang 'waiting_payment' dan dibuat lebih dari 24 jam lalu
...
if (empty($orders_to_cancel)) {
    echo "Tidak ada pesanan yang kedaluwarsa untuk dibatalkan.\n";
    echo "===== Selesai =====\n";
    exit;
}
...
foreach ($orders_to_cancel as $order) {
    ...
    try {
        ...
        $message = "Pesanan Anda #{$order_number} telah otomatis dibatalkan karena melewati batas waktu pembayaran 24 jam.";

...
// UPDATED: Menampilkan Nama Variasi di Ringkasan
...
// =================================================================
// OVERRIDE: Fetch cart dengan data Variasi
// =================================================================
function get_checkout_cart($conn, $user_id) {
    $sql = "SELECT c.*, 
    ...
    while ($row = $result->fetch_assoc()) {
        // Tentukan harga (variasi vs base)
        $price = (!empty($row['variation_id']) && !empty($row['variation_price'])) ? $row['variation_price'] : $row['base_price'];
    ...
    // Cari alamat default (sama seperti logic lama)
    ...
    if ($res_addr->num_rows > 0) $default_address = $res_addr->fetch_assoc();
    $stmt_addr->close();
...
$cart = get_checkout_cart($conn, $user_id);
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex-1">
                            ...
                            <?php if(!empty($item['variation_name'])): ?>
                                <p class="text-xs text-indigo-600 bg-indigo-50 inline-block px-1.5 py-0.5 rounded">

<?php
// File: admin/import/database_resi.php
// Halaman baru untuk menampilkan SEMUA data mentah dari tabel master 'imported_shipments'
...
// Filter Status Baru
$status_filter = sanitize_input($_GET['status'] ?? 'all'); // 'all', 'matched', 'unmatched'
...
<div class="bg-white shadow-md rounded-lg p-6">
    ...
    <h2 class="text-2xl font-semibold mb-4 text-gray-800">Database Resi</h2>
    <p class="mb-4 text-sm text-gray-600">Halaman ini berisi database semua resi unik yang pernah di-import dari Excel.</p>
    
    <!-- Filter Form -->
    <form method="GET" action="admin.php" class="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <input type="hidden" name="page" value="database_resi">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="status" id="status" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm sm:text-sm">
                    <option value="all" <?= $status_filter == 'all' ? 'selected' : '' ?>>Semua</option>
                    <option value="matched" <?= $status_filter == 'matched' ? 'selected' : '' ?>>Sudah Match</option>
                    <option value="unmatched" <?= $status_filter == 'unmatched' ? 'selected' : '' ?>>Belum Match</option>
                </select>
            </div>
        </div>
        <div class="text-right mt-4">
            <a href="admin.php?page=database_resi" class="text-sm text-gray-600 hover:text-gray-800 mr-4">Reset Filter</a>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status Match</th>
                    ...
                    <?php while ($row = $result_data->fetch_assoc()): ?>
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">Import: <?= date('d M Y', strtotime($row['imported_at'])) ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if ($row['order_id']): ?>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        <i class="fas fa-check-circle mr-1.5"></i>
                                        Matched (<?= htmlspecialchars($row['order_number']) ?>)
                                    </span>
                                <?php else: ?>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                            Tidak ada data resi ditemukan <?= !empty($search_query) ? 'untuk pencarian "' . htmlspecialchars($search_query) . '"' : '' ?>.
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
    ...
    <!-- Pagination -->
    <?php 
    $base_url = "?page=database_resi&q=" . urlencode($search_query) . "&status=" . urlencode($status_filter);
    if ($total_pages > 1) {
...
<div id="manualMatchModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden" onclick="closeManualMatchModal()">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 p-6" onclick="event.stopPropagation()">
        ...
        <form id="manualMatchForm" action="admin.php?page=database_resi" method="POST" class="mt-6 text-right">

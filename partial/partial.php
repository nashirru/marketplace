...
<footer class="bg-gray-800 text-white mt-12">
  <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div><h3 class="text-xl font-bold mb-4 text-indigo-400"><?= htmlspecialchars($store_name) ?></h3><p class="text-gray-400 text-sm"><?= htmlspecialchars($store_description) ?></p></div>
          <div><h3 class="text-lg font-semibold mb-4 text-indigo-400">Tautan</h3><ul class="space-y-2 text-sm">
              <li><a href="<?= BASE_URL ?>/" class="text-gray-400 hover:text-white transition">Beranda</a></li>
              <li><a href="<?= BASE_URL ?>/product/products.php" class="text-gray-400 hover:text-white transition">Semua Produk</a></li>
              <li><a href="<?= BASE_URL ?>/kategori/kategori.php" class="text-gray-400 hover:text-white transition">Kategori</a></li>
          </ul></div>
          <div><h3 class="text-lg font-semibold mb-4 text-indigo-400">Hubungi Kami</h3><ul class="space-y-2 text-sm">
            <?php if ($store_email): ?>
              <li><p class="text-gray-400"><i class="fas fa-envelope mr-2"></i><?= htmlspecialchars($store_email) ?></p></li>
            <?php endif; ?>
            <?php if ($store_address): ?>
              <li class="flex items-start"><i class="fas fa-map-marker-alt mr-2 mt-1"></i><p class="text-gray-400"><?= nl2br(htmlspecialchars($store_address)) ?></p></li>
            <?php endif; ?>
          </ul></div>
          <div><h3 class="text-lg font-semibold mb-4 text-indigo-400">Ikuti Kami</h3><div class="flex space-x-4">
            <?php if ($store_facebook): ?>
              <a href="<?= htmlspecialchars($store_facebook) ?>" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-indigo-500 transition"><i class="fab fa-facebook fa-lg"></i></a>
            <?php endif; ?>
            <?php if ($store_tiktok): ?>
              <a href="<?= htmlspecialchars($store_tiktok) ?>" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-indigo-500 transition"><i class="fab fa-tiktok fa-lg"></i></a>
            <?php endif; ?>
          </div></div>
      </div>
      <div class="mt-8 pt-4 border-t border-gray-700 text-center"><p class="text-sm text-gray-400">&copy; <?= date("Y") ?> <?= htmlspecialchars($store_name) ?>. All rights reserved.</p></div>
  </div>
</footer>
<?php
}
...
// Fungsi top_header() yang lama bisa dihapus karena tidak digunakan di file Anda
// function top_header($conn) {}
?>